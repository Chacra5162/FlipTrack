...
function _renderBundleList() {
  ...
  listEl.innerHTML = filtered.map(i => {
    ...
    return `<label style="display:flex;align-items:center;gap:8px;padding:8px 10px;border-bottom:1px solid var(--border);cursor:pointer;font-size:11px;overflow:hidden;box-sizing:border-box;width:100%">
      <input type="checkbox" ${checked} onchange="toggleBundleItem('${escAttr(i.id)}')" style="cursor:pointer;flex-shrink:0">
      ${thumb}
      <div style="flex:1;min-width:0;overflow:hidden">
        <div style="font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:100%">${escHtml(i.name || 'Untitled')}</div>
        <div style="font-size:10px;color:var(--muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escHtml(i.sku || '')}${i.category ? ' · ' + escHtml(i.category) : ''}</div>
      </div>
      <span style="color:var(--good);font-family:'DM Mono',monospace;font-weight:600;flex-shrink:0;white-space:nowrap">${fmt(i.price || 0)}</span>
    </label>`;
  }).join('');
