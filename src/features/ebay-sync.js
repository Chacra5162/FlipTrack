    ...
    while (hasMore) {
      ...
      for (const ebayItem of items) {
        ...
        if (local) {
          ...
          // Unmatched eBay item — import it into FlipTrack
          try {
            const product = ebayItem.product || {};
            const title = product.title || sku;
            const images = (product.imageUrls || []).filter(u => u && u.startsWith('http'));
            const qty = ebayItem.availability?.shipToLocationAvailability?.quantity || 1;

            // Fetch offer for this SKU to get listing ID, format, and price
            let listingId = '';
            let format = 'FIXED_PRICE';
            let price = 0;
            let bestOffer = false;
            try {
              const offerResp = await ebayAPI('GET',
                `${INVENTORY_API}/offer?sku=${encodeURIComponent(sku)}`
              );
              const offer = offerResp?.offers?.[0];
              if (offer) {
                ...
                bestOffer = !!offer.listingPolicies?.bestOfferTerms?.bestOfferEnabled;
              }
            } catch (_) { /* offer fetch failed — continue with defaults */ }
            ...
            const newItem = {
              ...
              category: '',
              platforms: ['eBay'],
              platformStatus: { eBay: qty > 0 ? 'active' : 'sold' },
              ...
              material: product.aspects?.Material?.[0] || '',
              images,
              image: images[0] || null,
              ebayItemId: sku,
              ebayListingId: listingId,
              ebayListingFormat: format,
              ebayBestOffer: bestOffer,
            ...
            logItemEvent(newId, 'ebay-sync', `Imported from eBay (${format === 'AUCTION' ? 'Auction' : 'Fixed Price'})`);
            addNotification('sync', 'eBay Import', `"${label}" imported from eBay`, newId);
            console.log('[eBay] Imported:', title, format === 'AUCTION' ? '(Auction)' : '');
            updated++;
          } catch (importErr) {
            console.warn('[eBay] Failed to import', sku, ':', importErr.message);
          }
          unmatched++;
        }
      }
