...
export async function generateHaulReceipt(haul) {
  ...
  const stats = [
    ...
    { label: 'INVESTED', value: fmt(Math.round(roi.totalCost)) },
    { label: 'EST. VALUE', value: fmt(Math.round(roi.totalRevenue)) },
    { label: 'PROFIT', value: fmt(Math.round(Math.abs(roi.profit))) },
    { label: 'ROI', value: ((roi.roi || 0) * 100).toFixed(0) + '%' },
