...
`).map(a=>a.replace(/^[-•*]\s*/,"").trim()).filter(Boolean):[]}}async function kg(t,e={}){const n=F(t);if(!n)throw new Error("Item not found");w("Generating AI listing…");const s=await ti(n,e);return(s.title||s.description)&&(n.aiListing||(n.aiListing={}),n.aiListing.title=s.title,n.aiListing.description=s.description,n.aiListing.keywords=s.keywords,n.aiListing.generatedAt=new Date().toISOString(),n.aiListing.platform=e.platform||"",j("inv",t),O(),w("AI listing generated ✓")),s}function Eg(t){var o;const e=t.aiListing||{},n=e.title||e.description;let s='<div class="ai-panel">';return n&&(s+=`
...
`)],{type:"text/csv;charset=utf-8;"}),s=URL.createObjectURL(n),o=document.createElement("a");o.href=s,o.download=`fliptrack-export-${ot()}.csv`,document.body.appendChild(o),o.click(),document.body.removeChild(o),URL.revokeObjectURL(s),w(`Exported ${K.size} item(s) to CSV ✓`)}let zt=[],nc="",va="spent",xn=0;const ea=50;let ha=null;const ur=[{name:"New",minSpent:0,color:"var(--muted)",icon:"👤"},{name:"Regular",minSpent:50,color:"var(--accent)",icon:"⭐"},{name:"VIP",minSpent:200,color:"var(--accent2)",icon:"💎"},{name:"Elite",minSpent:500,color:"var(--good)",icon:"👑"}];function Af(t){const e=B.filter(s=>s.buyerId===t).reduce((s,o)=>s+(o.price||0),0);let n=ur[0];for(const s of ur)e>=s.minSpent&&(n=s);return{...n,spent:e}}async function hy(){const t=await gt("buyers");try{zt=t?JSON.parse(t):[]}catch(e){console.warn("FlipTrack: corrupt buyers data:",e.message),zt=[]}}function Io(){et("buyers",JSON.stringify(zt)).catch(t=>console.warn("FlipTrack: buyers save failed:",t.message))}function sc(t){return zt.find(e=>e.id===t)}function by(){const t=document.getElementById("buyer_name"),e=document.getElementById("buyer_email"),n=document.getElementById("buyer_phone"),s=document.getElementById("buyer_ebay"),o=document.getElementById("buyer_posh"),a=document.getElementById("buyer_merc"),i=document.getElementById("buyer_notes"),r=(t.value||"").trim();if(!r){w("Name required",!0);return}const l={id:se(),name:r,handles:{eBay:(s.value||"").trim(),Poshmark:(o.value||"").trim(),Mercari:(a.value||"").trim()},email:(e.value||"").trim(),phone:(n.value||"").trim(),notes:(i.value||"").trim(),createdAt:Date.now()};zt.push(l),Io(),w(`${x(r)} added`),t.value="",e.value="",n.value="",s.value="",o.value="",a.value="",i.value="",cn()}function wy(t){const e=sc(t);if(!e||!confirm(`Delete ${x(e.name)}?`))return;const n=zt.indexOf(e);zt.splice(n,1),Io(),w(`${x(e.name)} deleted`),cn()}function xy(t){ha=ha===t?null:t,cn()}function $y(t){nc=(t||"").toLowerCase(),xn=0,cn()}function Sy(t){va=t,xn=0,cn()}function ky(t,e){if(!t)return null;const n=t.toLowerCase().trim();let s=zt.find(o=>o.name.toLowerCase()===n||Object.values(o.handles||{}).some(a=>a&&a.toLowerCase()===n));return s||(s={id:se(),name:t.trim(),handles:{[e]:t.trim()},email:"",phone:"",notes:"",createdAt:Date.now()},zt.push(s),Io()),s}function Ey(t,e){const n=B.find(s=>s.id===e);n&&(n.buyerId=t,O(),w("Sale linked to buyer"))}function Iy(t){const e=document.getElementById("comm_type_"+t),n=document.getElementById("comm_content_"+t),s=(e==null?void 0:e.value)||"note",o=((n==null?void 0:n.value)||"").trim();if(!o){w("Enter a message",!0);return}const a=sc(t);a&&(a.comms||(a.comms=[]),a.comms.push({type:s,content:o,date:Date.now()}),Io(),n&&(n.value=""),w("Note added"),cn())}function cn(){const t=document.getElementById("buyersContent");if(!t)return;const e=zt.length,n=zt.filter(c=>B.filter(u=>u.buyerId===c.id).length>1).length,s=zt.reduce((c,d)=>{const u=B.filter(p=>p.buyerId===d.id).reduce((p,y)=>p+(y.price||0),0);return c+u},0),o=B.length?s/B.length:0,a=zt.filter(c=>{const d=nc;return d?c.name.toLowerCase().includes(d)||c.email.toLowerCase().includes(d)||c.phone.includes(d)||c.handles&&Object.values(c.handles).some(u=>u.toLowerCase().includes(d)):!0});a.sort((c,d)=>{if(va==="name")return c.name.localeCompare(d.name);if(va==="recent")return d.createdAt-c.createdAt;const u=B.filter(y=>y.buyerId===c.id).reduce((y,v)=>y+(v.price||0),0);return B.filter(y=>y.buyerId===d.id).reduce((y,v)=>y+(v.price||0),0)-u});const i=a.slice(xn*ea,(xn+1)*ea);let r=`
  ...
  `,Ec=Nt.fresh||Nt.moderate||Nt.stale||Nt.deadPile?_("📅","Aging & Death Pile",kc,"var(--warn)"):"",Ic=m.length?_("🕸️","Not Moving — Consider Repricing",m.map(g=>A(g,b=>`
    ...
    <div class="pd-filters">
      ...
      <input class="pd-search" type="text" placeholder="Search items…" value="${x(uo)}" oninput="setProfitSearch(this.value)">
      <select class="pd-select" onchange="setProfitPlatFilter(this.value)">
        <option value="">All Platforms</option>
        ${c.map(p=>`<option value="${x(p)}"${fo===p?" selected":""}>${x(p)}</option>`).join("")}
      </select>
      <select class="pd-select" onchange="setProfitCatFilter(this.value)">
        <option value="">All Categories</option>
        ${d.map(p=>`<option value="${x(p)}"${mo===p?" selected":""}>${x(p)}</option>`).join("")}
      </select>
    </div>
  ...
  `}function Cy(t){$n=t,Es()}function By(t){uo=(t||"").trim(),Es()}function Ty(t){fo=t||"",Es()}function Py(t){mo=t||"",Es()}function My(t){wt===t?Kt*=-1:(wt=t,Kt=t==="name"?1:-1),Es()}let Je="",Sn="",kn="";function Dy(t){Je=(t||"").toLowerCase(),Fn()}function Ly(t){Sn=t||"",Fn()}function Ay(t){kn=t||"",Fn()}function zy(){Je="",Sn="",kn="",Fn()}const Wf={"Shipping Supplies":"exp-shipping","Packaging Materials":"exp-packaging","Platform Fees":"exp-fees",Sourcing:"exp-sourcing",Storage:"exp-storage",Printing:"exp-printing",Software:"exp-software",Marketing:"exp-marketing",Returns:"exp-returns","Office Supplies":"exp-office",Other:"exp-other"};function Gf(){const t=document.getElementById("exp_date");t&&!t.value&&(t.value=ot())}function Ry(){const t=document.getElementById("exp_date").value,e=document.getElementById("exp_cat").value,n=document.getElementById("exp_desc").value.trim(),s=document.getElementById("exp_amt"),o=Lt(s.value,{allowZero:!1});if(isNaN(o)&&s.value.trim()!==""){$e(s,{fieldName:"Amount"});return}if(!t){w("Please enter a date",!0);return}if(!n){w("Please enter a description",!0);return}if(!o||o<=0){w("Please enter a valid amount",!0);return}J.push({id:se(),date:t,category:e,description:n,amount:o}),O(),Fn(),document.getElementById("exp_desc").value="",document.getElementById("exp_amt").value="",vo.expense(),w("Expense added ✓")}async function Fy(t){if(!confirm("Delete this expense?"))return;const e=J.findIndex(n=>n.id===t);e>=0&&J.splice(e,1),O(),Fn(),w("Expense deleted"),await hs("ft_expenses",[t]),bs()}function Fn(){var r;const t=((r=document.getElementById("expCatFilt"))==null?void 0:r.value)||"all",e=document.getElementById("expBody"),n=document.getElementById("expEmpty"),s=document.getElementById("expTotalLbl"),o=[...J].filter(l=>t==="all"||l.category===t).filter(l=>!Je||(l.description||"").toLowerCase().includes(Je)||(l.category||"").toLowerCase().includes(Je)).filter(l=>!Sn||l.date>=Sn).filter(l=>!kn||l.date<=kn).sort((l,c)=>new Date(c.date)-new Date(l.date)),a=o.reduce((l,c)=>l+(c.amount||0),0);s&&(s.textContent=`${o.length} expense${o.length!==1?"s":""} · ${S(a)}`);const i=document.getElementById("expFilterBar");if(i&&(i.innerHTML=`<div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;padding:8px 0">
    ...
    </div>`),!o.length){e&&(e.innerHTML=""),n&&(n.style.display="block");return}n&&(n.style.display="none"),e.innerHTML=o.map(l=>{const c=Wf[l.category]||"exp-other",d=x(l.description),u=Bn(l.description),p=Bn(l.id);return`<tr>
...
Ships fast! Check my store for more.`,platforms:["Whatnot"],tags:["whatnot","fixed","store"],isDefault:!0}];let ue=0;const Ks=25;let go="",fs="all",le="none",wa=null,Js=null,Qe=null,Pe=null,ms="all",ye=null,_o=!1,Ut="shows",xa="",$a="",Sa="",ka="",ne=new Set,Me="overview";function U(){const t=document.getElementById("crosslistContent");if(!t)return;Hd();const e=k.filter(i=>(i.qty||0)>0),n=Wd(e),s=Yr(e,7),o=An(e);let a="";if(a+=ym(),a+=im(),a+=pm(),a+=`<div class="cl-stats-strip">
    ...
    <div class="form-grid" style="gap:6px">
      ...
      <select id="clBulkPlat" class="fgrp" style="grid-column:1/-1">
        <option value="">All Platforms</option>
        ${Object.keys(bo).slice(0,15).map(a=>`<option value="${x(a)}">${x(a)}</option>`).join("")}
      </select>
    ...
    <input type="text" placeholder="Search items..." value="${x(go)}"
    ...
    <select onchange="clSetPlatFilter(this.value)" class="cl-filter-select">
      ...
      ${n.map(l=>`<option value="${x(l)}" ${fs===l?"selected":""}>${x(l)}</option>`).join("")}
    </select>
    ...
    <p style="color:var(--muted);font-size:12px;margin-bottom:12px">Templates auto-fill title & description when listing. Use placeholders: {name}, {condition}, {category}, {price}, {notes}, {upc}, {author}, {isbn}</p>`;if(!t.length)e+='<div style="text-align:center;padding:24px;color:var(--muted)">No templates yet. Click "+ New Template" to create one.</div>';else{e+='<div class="cl-template-grid">';for(const n of t)e+=`<div class="cl-template-card">
      ...
      </div>`;e+="</div>"}return e+="</div>",e}function Ky(t){Me=t,ue=0,U()}function Jy(t){go=t||"",ue=0,U()}function Qy(t){fs=t||"all",ue=0,U()}function Xy(t){ms=t||"all",ue=0,U()}function Zy(t,e){wo(t,e),O(),V(),w(`Relisted on ${e} ✓`),U()}function tv(t,e){Tt(t,e,"delisted"),O(),V(),w(`Delisted from ${e}`),U()}function ev(t,e){const n=k.find(r=>r.id===t);if(!n)return;const o=(n.platformStatus||{})[e]||"active",a=["active","sold","delisted","expired","draft"],i=a[(a.indexOf(o)+1)%a.length];Tt(t,e,i),i==="active"&&Ue(t,e,ot()),O(),V(),U()}function nv(t,e){const n=k.find(o=>o.id===e);if(!n)return;const s=Qr(t,n);window.open(s,"_blank")}function sv(t){const e=k.find(n=>n.id===t);e&&Qd(e)}function ov(){const t=k.filter(n=>(n.qty||0)>0),e=An(t);if(!e.length){w("No expired listings to relist");return}if(confirm(`Relist ${e.length} expired listing(s)?`)){for(const{item:n,platform:s}of e)wo(n.id,s);O(),V(),w(`${e.length} listing(s) relisted ✓`),U()}}function av(){const t="clTemplateModal-"+Date.now(),e=`<div id="${t}" style="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);display:flex;align-items:center;justify-content:center;z-index:var(--z-modal)" onclick="if(event.target.id==='${t}') document.getElementById('${t}').remove()">
...
{notes}</textarea>
  ...
  </div>`;document.body.insertAdjacentHTML("beforeend",e),document.getElementById("clTplName").focus()}async function iv(t){var a,i,r,l,c,d,u,p,y;const e=(i=(a=document.getElementById("clTplName"))==null?void 0:a.value)==null?void 0:i.trim(),n=((l=(r=document.getElementById("clTplCat"))==null?void 0:r.value)==null?void 0:l.trim())||"",s=(d=(c=document.getElementById("clTplTitle"))==null?void 0:c.value)==null?void 0:d.trim(),o=((p=(u=document.getElementById("clTplDesc"))==null?void 0:u.value)==null?void 0:p.trim())||"";if(!e){w("Enter a template name",!0);return}if(!s){w("Enter a title formula",!0);return}(y=document.getElementById(t))==null||y.remove(),em({name:e,category:n,titleFormula:s,descriptionTemplate:o}),w("Template created ✓"),U()}function rv(t){confirm("Delete this template?")&&(nm(t),w("Template deleted"),U())}function im(){const t=dt(),e=nl(),n=Sp(),s=kp(),o=s?new Date(s).toLocaleString([],{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}):"Never";if(t){let a=`<div class="etsy-panel etsy-connected">
    ...
    <select id="etsyTagItemPick" onchange="clEtsyTagSelect(this.value)" style="width:100%;padding:6px;border-radius:6px;border:1px solid var(--border);background:var(--bg);color:var(--text);margin-bottom:10px">
      ...
      ${t.map(n=>`<option value="${n.id}" ${Pe===n.id?"selected":""}>${x(n.name||"Untitled")} (${(n.tags||[]).length}/13 tags)</option>`).join("")}
        ...
        <div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:4px">
          ${s.map((a,i)=>`<span class="etsy-tag-chip" onclick="clEtsyRemoveTag('${Pe}',${i})">${x(a)} ×</span>`).join("")}
          ...
          <div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:4px">
            ${o.slice(0,13-s.length).map(a=>`<span class="etsy-tag-suggest" onclick="clEtsyAddTag('${Pe}','${x(a)}')">${x(a)} +</span>`).join("")}
          </div>
      ...
      </div>`}}return e+="</div>",e}function lv(){tl()}function cv(){confirm("Disconnect your Etsy shop? You can reconnect anytime.")&&el()}async function dv(){w("Syncing Etsy listings…"),U();try{const t=await ol();w(`Etsy sync complete — ${t.matched} matched, ${t.updated} updated`)}catch(t){w(`Etsy sync error: ${t.message}`,!0)}U()}async function pv(t){(await al(t)).success&&U()}async function uv(t){if(!confirm("Deactivate this Etsy listing?"))return;(await xp(t)).success&&(V(),U())}async function fv(t){if(!confirm("Renew this Etsy listing? ($0.20 fee applies)"))return;(await $p(t)).success&&(V(),U())}function mv(t){le=le===t?"none":t,U()}async function gv(){w("Loading Etsy stats…");try{wa=await Bp(),U()}catch(t){w(`Stats error: ${t.message}`,!0)}}async function yv(){w("Loading Etsy reviews…");try{Js=await Pp(),U()}catch(t){w(`Reviews error: ${t.message}`,!0)}}async function vv(){w("Loading pending shipments…");try{Qe=await il(),U()}catch(t){w(`Shipments error: ${t.message}`,!0)}}async function hv(t){var i;const e=document.getElementById(`etsyTrack_${t}`),n=document.getElementById(`etsyCarrier_${t}`),s=(i=e==null?void 0:e.value)==null?void 0:i.trim(),o=(n==null?void 0:n.value)||"usps";if(!s){w("Enter a tracking number",!0);return}(await Dp(t,s,o)).success&&(Qe=await il(),U())}async function bv(){w("Syncing quantities…");try{const t=await _p();w(`Qty sync: ${t.pulled} pulled, ${t.pushed} pushed`),U()}catch(t){w(`Qty sync error: ${t.message}`,!0)}}async function wv(t){w("Uploading photos…");try{await Cp(t)}catch(e){w(`Photo error: ${e.message}`,!0)}}async function xv(t){await Ep(t)}async function $v(t){w("Pushing price to Etsy…");try{(await rl(t)).success?w("Price synced to Etsy"):w("Price sync failed",!0)}catch(e){w(`Price sync error: ${e.message}`,!0)}}function Sv(t){Pe=t||null,U()}function kv(t,e){const n=k.find(s=>s.id===t);!n||!n.tags||(n.tags.splice(e,1),j("inv",t),O(),U())}function Ev(t,e){const n=k.find(s=>s.id===t);if(n){if(n.tags||(n.tags=[]),n.tags.length>=13){w("Max 13 tags",!0);return}n.tags.includes(e)||(n.tags.push(e.slice(0,20)),j("inv",t),O(),U())}}async function Iv(t){ll(t).length||w("No more suggestions"),U()}async function _v(t){w("Saving tags to Etsy…");try{const e=k.find(n=>n.id===t);if(!e)return;await zp(t,e.tags||[])}catch(e){w(`Tag save error: ${e.message}`,!0)}}async function Cv(){w("Syncing Etsy expenses…");try{const t=await Fp();w(`${t.synced} expense${t.synced!==1?"s":""} synced`)}catch(t){w(`Expense sync error: ${t.message}`,!0)}}function Bv(t){t?Gd():Vd(),U()}function Tv(){la()===0&&w("No expired renewable listings to relist"),U()}function Pv(){var i,r,l,c,d;const t=((i=document.getElementById("clBulkCat"))==null?void 0:i.value)||"",e=((r=document.getElementById("clBulkPlat"))==null?void 0:r.value)||"",n=((l=document.getElementById("clBulkType"))==null?void 0:l.value)||"percent",s=parseFloat((c=document.getElementById("clBulkVal"))==null?void 0:c.value)||0,o=parseInt((d=document.getElementById("clBulkMinDays"))==null?void 0:d.value)||0;if(!s){w("Enter a price adjustment value",!0);return}const a=n==="percent"?`${s>0?"+":""}${s}%`:`${s>0?"+":""}$${s}`;confirm(`Apply ${a} to matching items?`)&&(Kd({category:t,platform:e,adjustType:n,adjustValue:s,minDaysListed:o}),U())}function pm(){const t=vf(),n=jl().length;let s=`<div class="wn-panel">
  ...
  </head><body>${e}</body></html>`),n.document.close(),n.print()}function Qv(t){window.exportShowResultsCSV?window.exportShowResultsCSV(t):w("CSV export loading…")}function Xv(t){ne.has(t)?ne.delete(t):ne.add(t),U()}function Zv(){const t=Kl(30),e=Jl();ne.clear(),t.slice(0,e.recommended).forEach(n=>ne.add(n.item.id)),U()}function th(){ne.clear(),U()}async function eh(){if(ne.size===0){w("No items selected",!0);return}const t=prompt("Show name:",`Smart Show - ${new Date().toLocaleDateString()}`);if(!t)return;const e=prompt("Date (YYYY-MM-DD):",ot());if(!e)return;const n=prompt("Time (HH:MM):","19:00")||"",s=await bi(t,e,n,"",{items:[...ne]});ne.clear(),Ut="shows",ye=s.id,U(),w(`Show created with ${s.items.length} items`)}function ym(){const t=ft(),e=Gp(),n=uu(),s=fu(),o=s?new Date(s).toLocaleString([],{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}):"Never";return!t&&!Wp()?(da(),`<div class="ebay-panel">
...
`),o=new Blob([s],{type:"text/csv"}),a=URL.createObjectURL(o),i=document.createElement("a");i.href=a,i.download=`ship-log-${ot()}.csv`,i.click(),URL.revokeObjectURL(a),w("Exported ship log")}async function Mt(){await km();const t=document.getElementById("view-shipping");if(!t)return;const e=ki(),n=Math.max(1,Math.ceil(e.length/js));Xt=Math.max(0,Math.min(Xt,n-1));const s=Xt*js,o=s+js,a=e.slice(s,o),i=B.filter(u=>!u.shipped).length,r=B.filter(u=>{if(!u.shippedDate)return!1;const p=new Date(u.shippedDate).getTime();return Date.now()-p<864e5}).length,l=(()=>{const u=B.filter(y=>y.shippedDate&&y.date);if(u.length===0)return 0;const p=u.reduce((y,v)=>{const h=new Date(v.date).getTime(),m=new Date(v.shippedDate).getTime();return y+(m-h)},0);return Math.round(p/u.length/864e5)})(),c=B.filter(u=>u.shipped).reduce((u,p)=>u+(p.actualShipCost||0),0),d=a.length>0&&a.every(u=>$t.has(u.id));t.innerHTML=`
    <div class="panel">
      ...
      <div style="padding:14px;border-bottom:1px solid var(--border);background:var(--surface2)">
        <div class="form-grid" style="margin-bottom:12px">
          <input type="text" placeholder="Search item or buyer..." value="${x(Cn)}"
  ...
  `)}function Qs(t,e,n){if(!t||!t.itemIds||!t.itemIds.length)return{totalRevenue:0,totalCost:0,profit:0,roi:0,margin:0};let s=0,o=t.totalSpent||0;t.itemIds.forEach(l=>{if(!e.find(p=>p.id===l))return;const u=n.filter(p=>p.itemId===l).reduce((p,y)=>p+(y.amount||0),0);s+=u});const a=s-o,i=o?a/o:0,r=s?a/s:0;return{totalRevenue:s,totalCost:o,profit:a,roi:i,margin:r}}function mc(t,e){return!t||!t.itemIds?[]:t.itemIds.map(n=>e.find(s=>s.id===n)).filter(Boolean)}function Im(t,e){const n=mc(t,e);if(!n.length)return{};const s=(t.totalSpent||0)/n.length,o={};return n.forEach(a=>{o[a.id]=s}),o}function _m(t,e,n){const s={};return t.forEach(o=>{const a=o.location||"Unknown";s[a]||(s[a]={location:a,count:0,totalSpent:0,totalRevenue:0,itemCount:0,roi:0,margin:0});const i=Qs(o,e,n);s[a].count+=1,s[a].totalSpent+=o.totalSpent||0,s[a].totalRevenue+=i.totalRevenue,s[a].itemCount+=o.itemIds?o.itemIds.length:0}),Object.values(s).forEach(o=>{o.totalSpent&&(o.roi=(o.totalRevenue-o.totalSpent)/o.totalSpent,o.margin=o.totalRevenue?(o.totalRevenue-o.totalSpent)/o.totalRevenue:0)}),s}function Cm(t,e,n,s=5){const o=_m(t,e,n);return Object.values(o).filter(a=>a.count>0).sort((a,i)=>i.roi-a.roi).slice(0,s)}function Bm(t){const e=t.length,n=t.reduce((o,a)=>o+(a.totalSpent||0),0),s=t.reduce((o,a)=>o+(a.itemIds?a.itemIds.length:0),0);return{totalHauls:e,totalInvested:n,avgPerHaul:e?n/e:0,totalItems:s,avgItemsPerHaul:e?s/e:0}}let at=[],Zn="",Dt="date-desc",Ia=null,Xe=0,Vn=25;async function Lh(){try{const t=ct(),e=St();if(t&&e){const{data:n,error:s}=await t.from("ft_hauls").select("id, data").eq("account_id",e.id);if(!s&&n&&n.length>0){at=n.map(o=>({id:o.id,...o.data})),await et("hauls",at),localStorage.setItem("ft_hauls",JSON.stringify(at));return}}}catch(t){console.warn("FlipTrack: Hauls cloud pull failed, using local:",t.message)}try{at=await gt("hauls"),at||(at=[])}catch(t){console.warn("FlipTrack: Hauls IDB load failed, using localStorage:",t.message);try{at=JSON.parse(localStorage.getItem("ft_hauls")||"[]")}catch{at=[]}}at.length>0&&gc().catch(t=>console.warn("FlipTrack: Hauls initial push error:",t.message))}async function Co(){await et("hauls",at),localStorage.setItem("ft_hauls",JSON.stringify(at)),gc().catch(t=>console.warn("FlipTrack: Hauls cloud save error:",t.message))}async function gc(){const t=ct(),e=St();if(!t||!e||!navigator.onLine)return;const n=e.id,s=at.map(i=>{const{id:r,...l}=i;return{id:r,account_id:n,data:l,updated_at:new Date().toISOString()}});if(s.length>0){const{error:i}=await t.from("ft_hauls").upsert(s,{onConflict:"id"});if(i)throw new Error(i.message)}const o=at.map(i=>i.id),{data:a}=await t.from("ft_hauls").select("id").eq("account_id",n);if(a){const i=a.filter(r=>!o.includes(r.id)).map(r=>r.id);i.length>0&&await t.from("ft_hauls").delete().in("id",i)}}function Ce(){const t=document.getElementById("sourcingContent");if(!t)return;const e=at.filter(o=>!Zn||(o.location||"").toLowerCase().includes(Zn)||(o.notes||"").toLowerCase().includes(Zn)).sort((o,a)=>{if(Dt==="date-desc")return new Date(a.date)-new Date(o.date);if(Dt==="date-asc")return new Date(o.date)-new Date(a.date);if(Dt==="spent-high")return(a.totalSpent||0)-(o.totalSpent||0);if(Dt==="spent-low")return(o.totalSpent||0)-(a.totalSpent||0);if(Dt==="roi-high"){const i=Qs(o,k,B).roi;return Qs(a,k,B).roi-i}return Dt==="location"?(o.location||"").localeCompare(a.location||""):new Date(a.date)-new Date(o.date)}),n=Bm(at),s=Cm(at,k,B,3);t.innerHTML=`
    <div class="sourcing-container" style="display:flex;flex-direction:column;gap:16px;padding:12px">
      ...
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;padding:0 0 8px 0">
        <input type="text" placeholder="Search location, notes..." value="${x(Zn)}"
  ...
  `,n.appendChild(s),document.body.appendChild(n),window._itemLinkModal=n}async function Oh(t){const e=window._itemLinkModal;if(!e)return;const n=e.querySelectorAll('input[type="checkbox"]:checked'),s=Array.from(n).map(a=>a.value),o=at.find(a=>a.id===t);o&&(o.itemIds.push(...s),await Co(),w(`Linked ${s.length} item${s.length!==1?"s":""}`)),Tm(),Ce()}function Tm(){const t=window._itemLinkModal;t&&(t.remove(),window._itemLinkModal=null)}async function Nh(t,e){const n=at.find(s=>s.id===t);if(n){const s=n.itemIds.indexOf(e);s>=0&&(n.itemIds.splice(s,1),await Co(),Ce())}}function Uh(t){Zn=(t||"").toLowerCase(),Xe=0,Ce()}function qh(t){Dt=t||"date-desc",Xe=0,Ce()}const Pm={2024:.67,2025:.7,2026:.7};let fe=[];async function jh(){try{fe=await gt("mileage_log")||[]}catch(t){console.warn("Failed to load mileage log:",t.message),fe=[]}}async function Mm(t){if(!t.date||!t.miles||!t.purpose)return w("Please fill in date, miles, and purpose",!0),!1;const e=Number(t.miles)||0;if(e<=0)return w("Miles must be greater than 0",!0),!1;const n={id:se(),date:t.date,miles:e,purpose:t.purpose.trim(),startLocation:(t.startLocation||"").trim(),endLocation:(t.endLocation||"").trim(),linkedHaulId:t.linkedHaulId||null};fe.push(n),fe.sort((s,o)=>new Date(o.date)-new Date(s.date));try{return await et("mileage_log",fe),!0}catch(s){return console.error("Failed to save mileage entry:",s.message),w("Failed to save mileage entry",!0),fe.pop(),!1}}async function Dm(t){const e=fe.findIndex(n=>n.id===t);if(e===-1)return!1;fe.splice(e,1);try{return await et("mileage_log",fe),!0}catch(n){return console.error("Failed to delete mileage entry:",n.message),w("Failed to delete mileage entry",!0),!1}}function Lm(t){const e=Pm[t]||.7,n=fe.filter(a=>new Date(a.date).getFullYear()===t),s=n.reduce((a,i)=>a+(i.miles||0),0),o=s*e;return{totalMiles:s,totalDeduction:o,rate:e,entries:n}}function Hh(){const t=new Date().getFullYear(),e=Lm(t),n=e.entries.sort((s,o)=>new Date(o.date)-new Date(s.date)).map((s,o)=>`
      <tr>
  ...
  `:'<div class="pt-empty">No image to edit. Add a photo first.</div>'}function Is(t){return new Promise((e,n)=>{const s=new Image;s.crossOrigin="anonymous",s.onload=()=>e(s),s.onerror=()=>n(new Error("Failed to load image")),s.src=t})}let On=[],sn=new Set,Ln=null,tn={current:0,total:0,status:"idle"};function a0(t){On=t.filter(e=>{const n=F(e);return n&&(n.qty||0)>0}),sn.clear(),Ln=null,tn={current:0,total:0,status:"idle"}}function i0(t){sn.has(t)?sn.delete(t):sn.add(t)}function Om(){return{items:On.map(t=>F(t)).filter(Boolean),platforms:[...sn],template:Ln,progress:{...tn}}}async function r0(t={}){var l;const e=t.delayMs||2e3,n=[...sn],s=On.map(c=>F(c)).filter(Boolean);if(!s.length||!n.length)return w("Select items and platforms first",!0),{success:0,failed:0,skipped:0};tn={current:0,total:s.length*n.length,status:"running"};let a=0,i=0,r=0;for(const c of s)for(const d of n){if(tn.current++,t.onProgress&&t.onProgress(tn),((l=c.platformStatus)==null?void 0:l[d])==="active"){r++;continue}try{if(d==="eBay"&&ft())(await oi(c.id)).success&&c.price>0&&await So(c.id),a++;else{const u=Ln?{titleFormula:Ln}:null,{title:p,description:y}=Va(c,u);await navigator.clipboard.writeText(`${p}

${y}`);const v=Qr(d,c);window.open(v,"_blank"),Tt(c.id,d,"draft"),Ue(c.id,d,ot()),j("inv",c.id),a++,e>0&&await new Promise(h=>setTimeout(h,e))}}catch(u){console.warn(`Batch list error for ${c.name} on ${d}:`,u.message),i++}}return O(),V(),tn.status="done",w(`Batch listing complete: ${a} listed, ${r} skipped, ${i} failed`),{success:a,failed:i,skipped:r}}function Nm(){return On.map(n=>F(n)).filter(Boolean).map(n=>{const{title:s,description:o}=Va(n,Ln);return`=== ${s} ===
${o}
`}).join(`
---

`)}async function l0(){const t=Nm();try{await navigator.clipboard.writeText(t),w(`Copied listing text for ${On.length} items ✓`)}catch{w("Copy failed",!0)}}function c0(){const t=Om();if(!t.items.length)return`<div class="bl-empty">
